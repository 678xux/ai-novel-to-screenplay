...
def chunk_paragraphs(paragraphs: list[str], chunk_size: int) -> list[list[str]]:
    chunks = [paragraphs[index : index + chunk_size] for index in range(0, len(paragraphs), chunk_size)]
    return chunks or [[]]
...
def chapter_to_scenes(chapter: dict, chapter_index: int, density: str, known_names: list[str]) -> list[dict]:
    ...
    chunk_size = 4 if density == "detailed" else 8 if density == "compact" else 6
    scenes = []
    for scene_index, group in enumerate(chunk_paragraphs(paragraphs, chunk_size), start=1):
        scene_text = "\n".join(group)
