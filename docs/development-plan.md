...
18. 篇幅规划与节奏估算
    - 范围：为幕、场景和 `production_notes.runtime_plan` 增加预计时长、场均时长、最长/最短场和节奏建议，并在质量报告、大纲视图、Markdown 导出和 Schema 文档中同步。
...
20. 来源章节覆盖报告
    - 范围：在 `production_notes.source_coverage` 中记录每章对应的场景、节拍、角色、道具和覆盖状态，并在质量报告、大纲视图、Markdown 导出和 Schema 文档中展示。
    - 测试：转换结果应通过 Schema 校验，每个示例章节都应有覆盖记录，Markdown 和 e2e 检查应包含来源覆盖信息。
    - 目的：服务用户下载多章小说后的真实测试场景，帮助快速确认没有漏章、空章或未转成可编辑场景的章节。

21. 结构化修订任务清单
    - 范围：在 `production_notes.revision_tasks` 中生成带优先级、类别、目标场景、原因和动作建议的修订任务，并在质量报告、大纲视图、Markdown 导出和 Schema 文档中展示。
    - 测试：转换结果应通过 Schema 校验，示例结果应生成修订任务，YAML、Markdown 和 e2e 检查应包含修订任务信息。
    - 目的：把“AI 辅助初稿”延伸到“下一步如何打磨”，让作者能直接定位需要补对白、校准说话人、视觉化旁白、拆分长场景或检查人物功能的场景。
